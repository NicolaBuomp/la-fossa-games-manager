    ...
    <lfg-modal [open]="modalOpen()" [title]="editing() ? 'Modifica entrata' : 'Nuova entrata'" (close)="modalOpen.set(false)">
      <form class="grid gap-4" (ngSubmit)="save()">
        <fieldset [disabled]="saving()" class="grid gap-4 disabled:opacity-70">
          <label class="grid gap-1 text-sm font-bold">Fonte <input required name="source" [(ngModel)]="form.source" class="rounded-lg border border-black/10 bg-neutral-50 px-3 py-3 font-normal disabled:cursor-not-allowed disabled:opacity-70"></label>
          ...
          <label class="grid gap-1 text-sm font-bold">Incassato da
            <select name="received_by" [(ngModel)]="form.received_by" class="rounded-lg border border-black/10 bg-neutral-50 px-3 py-3 font-normal disabled:cursor-not-allowed disabled:opacity-70">
