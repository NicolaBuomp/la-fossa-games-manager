  ...
  template: `
    <section class="space-y-4">
      <div class="flex flex-wrap items-end justify-between gap-3">
        <div><p class="text-xs font-bold uppercase tracking-[0.18em] text-neutral-500">Modulo sponsor</p><h1 class="font-display text-3xl uppercase">Sponsor</h1></div>
        <div class="flex gap-2"><button class="rounded-lg bg-white px-4 py-2 text-sm font-bold ring-1 ring-black/10" (click)="export()">CSV</button><button class="rounded-lg bg-ink px-4 py-2 text-sm font-bold text-white" (click)="newItem()">Nuovo</button></div>
      </div>
      ...
      @if (!items().length) { <lfg-empty-state title="Nessuno sponsor" text="Aggiungi aziende, contatti, valore e stato della trattativa." /> }
      @else {
          ...
          @for (item of items(); track item.id) {
            <article class="rounded-lg border border-black/10 bg-white p-4">
              ...
              <div class="mt-4 flex flex-wrap justify-between gap-2 border-t border-black/5 pt-3">
                ...
                <div class="flex gap-2"><button class="rounded-md bg-neutral-100 px-3 py-1.5 text-xs font-bold uppercase" (click)="edit(item)">Modifica</button>@if (auth.isAdmin()) { <button class="rounded-md bg-red-50 px-3 py-1.5 text-xs font-bold uppercase text-red-700" (click)="remove(item)">Elimina</button> }</div>
              </div>
            </article>
          }
        </div>
    ...
    <lfg-modal [open]="modalOpen()" [title]="editing() ? 'Modifica sponsor' : 'Nuovo sponsor'" (close)="modalOpen.set(false)">
      <form class="grid gap-4" (ngSubmit)="save()">
        ...
        <div class="grid gap-3 sm:grid-cols-2"><label class="grid gap-1 text-sm font-bold">Referente <input name="contact_name" [(ngModel)]="form.contact_name" class="rounded-lg border border-black/10 bg-neutral-50 px-3 py-3 font-normal"></label><label class="grid gap-1 text-sm font-bold">Contatto <input name="contact_info" [(ngModel)]="form.contact_info" class="rounded-lg border border-black/10 bg-neutral-50 px-3 py-3 font-normal"></label></div>
        <div class="grid gap-3 sm:grid-cols-3"><label class="grid gap-1 text-sm font-bold">Valore <input type="number" step="0.01" name="value" [(ngModel)]="form.value" class="rounded-lg border border-black/10 bg-neutral-50 px-3 py-3 font-normal"></label><label class="grid gap-1 text-sm font-bold">Tipo <select name="type" [(ngModel)]="form.type" class="rounded-lg border border-black/10 bg-neutral-50 px-3 py-3 font-normal"><option value="cash">Cash</option><option value="in_natura">In natura</option></select></label><label class="grid gap-1 text-sm font-bold">Stato <select name="status" [(ngModel)]="form.status" class="rounded-lg border border-black/10 bg-neutral-50 px-3 py-3 font-normal">@for (s of statuses; track s.id) { <option [value]="s.id">{{ s.label }}</option> }</select></label></div>
...
export class SponsorsComponent implements OnInit {
  ...
  async setStatus(item: Sponsor, status: SponsorStatus): Promise<void> { try { await this.service.update(item.id, { status }); await this.load(); } catch (e) { this.error.set(this.message(e)); } }
  async remove(item: Sponsor): Promise<void> { if (!confirm(`Eliminare lo sponsor "${item.company_name}"?`)) return; try { await this.service.remove(item.id); await this.load(); } catch (e) { this.error.set(this.message(e)); } }
