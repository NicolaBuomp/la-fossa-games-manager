...
@Component({
  ...
  template: `
    <main
      class="grid min-h-screen place-items-center bg-app px-4 py-8 text-primary"
    >
      <section
      ...
      >
        ...
        <form class="mt-6" (ngSubmit)="submit()">
          <fieldset [disabled]="loading" class="space-y-4 disabled:opacity-70">
            ...
            <button
              class="bg-strong text-on-strong w-full rounded-lg px-4 py-3 text-sm font-bold uppercase tracking-wide transition hover:opacity-90 disabled:opacity-60"
            >
